class Validation {
    ...
    constructor(options) {
        ...
        this.rulesList = new Map([
            ...
            ['radio-required', {
                checkEvents: ['change'],
                errorMessage: '',
                functionCheck: this.validateRadioRequired.bind(this),
            }],
            ['min-count-symbol', {
                checkEvents: ['change'],
    ...
    validateRadioRequired($field, $elem, errorMessage) {

        let name = $elem.name;
        let validate = false;

        for (let $input of this.$form.querySelectorAll('input[name="'+name+'"]')) {
