...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	foreach ($arResult['SECTIONS'] as $key1 => $arSection1)
	{
		if( $arSection1['DEPTH_LEVEL'] == "1" ){
			...
			$arResult['SECTIONS_TREE_MAIN'][$arSection1['ID']] = $arSection1;
		}elseif( $arSection1['DEPTH_LEVEL'] == "2" ){
			$sub_section_id = $arSection1["ID"];
			$arResult['SECTIONS_TREE'][$section_id][] = $arSection1;
		}elseif( $arSection1['DEPTH_LEVEL'] == "3" ){
			$arResult['SECTIONS_TREE_2'][$sub_section_id][] = $arSection1;
		}
		...
		if( $arSection1['DEPTH_LEVEL'] == "2" ){
			$subsection_count = 0;
			$section_key = $arSection1['ID'];
		}
		
		if( $arSection1['DEPTH_LEVEL'] == "3" ){
			$subsection_count++;
			$arResult['SUB_SECTIONS_NUM'][$section_key]['COUNT'] = $subsection_count;
		}
