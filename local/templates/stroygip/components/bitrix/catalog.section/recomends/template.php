        ...
        <div class="owl-carousel owl-theme owl-carousel-style recommends-owl-carousel">
            <?foreach ($arResult['ITEMS'] as $arItem)
            {
                ...
                if($arItem['PREVIEW_PICTURE']['ID']){
                    $preview_picture = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']["ID"],array("width" => 240, "height" => 208),BX_RESIZE_IMAGE_PROPORTIONAL)['src'];
                }else{
                ...
                <div class="catalog-category__item"  id="<?=$areaIds[$arItem['ID']];?>">
                    <div class="catalog-category__button">
                        <a class="button button--red" href="javascript:void(0)" data-action="add2basket" data-id="<?=$arItem['ID']?>">В корзину</a></div>
                    <a class="catalog-category__image-box" href="<?=$arItem['DETAIL_PAGE_URL']?>">
                        ...
                        <?if($arResult['MARKS'][$arItem['PROPERTIES']['MARK']['VALUE']]){?>
                            <div class="catalog-category__mark">
                                <?if($arResult['MARKS'][$arItem['PROPERTIES']['MARK']['VALUE']]['UF_FILE']['SRC']){?>
                                    <img class="catalog-category__mark-icon" src="<?=$arResult['MARKS'][$arItem['PROPERTIES']['MARK']['VALUE']]['UF_FILE']['SRC']?>">
                                <?}?>
                        ...
                        <div class="catalog-category__icon-box">
                            <div class="catalog-category__icon">
                                <div class="catalog-category__icon-image catalog-category__icon-image--favourite<?=$fav_act?>" data-action="<?=$fav_action?>" data-id="<?=$arItem['ID']?>" data-add="FAVORITES"></div>
                            </div>
                            <div class="catalog-category__icon">
                                <div class="catalog-category__icon-image catalog-category__icon-image--comparison<?=$comp_act?>" data-action="<?=$comp_action?>" data-id="<?=$arItem['ID']?>" data-add="COMPARE"></div>
                            </div>
                        </div>
                    </a>
                    <div class="catalog-category__price-box">
                        <?if($arItem['MIN_PRICE']['DISCOUNT_DIFF']>0){?>
                            <div class="catalog-category__discount"><?=number_format($arItem['MIN_PRICE']['VALUE'],'0',',',' ')?><span class="catalog-category__discount-span"><?=CURRENCY;?></span></div>
                        <?}?>
                        <div class="catalog-category__price"><?=$arItem['MIN_PRICE']['DISCOUNT_DIFF']==0?number_format($arItem['MIN_PRICE']['DISCOUNT_VALUE'],'0',',',' '):number_format($arItem['MIN_PRICE']['VALUE'],'0',',',' ')?><span class="catalog-category__price-span"><?=CURRENCY;?></span></div>
                    </div>
                    <?if($arItem['PROPERTIES']['SIZE']['VALUE']){?>
                        <div class="catalog-category__size"><?=$arItem['PROPERTIES']['SIZE']['VALUE']?></div>
                    <?}?>
                    <a class="catalog-category__name" href="<?=$arItem['DETAIL_PAGE_URL']?>"><?=$arItem['NAME']?></a>
                    <div class="catalog-category__container">
                        ...
                        <?if($arItem['PROPERTIES']['ART_NUMBER']['VALUE']){?>
                            <div class="catalog-category__article">Арт. <?=$arItem['PROPERTIES']['ART_NUMBER']['VALUE']?></div>
                        <?}?>
                    </div>
                </div>
            <?}?>
